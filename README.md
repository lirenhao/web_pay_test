...
#### 商户登陆
- 商户登陆功能
  - 商户登陆成功：输入“商户的用户名称”，选择终端类型为商户端，页面会跳转进入“创建订单”页面
  - 商户登陆失败：“商户的用户名称”文本框不输入数据，选择终端类型为商户端，登陆页面的“登陆按钮”不可以点击。
...
- 点击“提交”按钮：
   ...
   - 当录入商品信息完整时，会成功提交订单，结果页面跳转进入“订单页面”
- 点击“订单待支付”按钮：页面跳转到待支付的“订单页面”

#### 商户查看订单
- 前提：商户创建订单成功，会跳转到当前订单信息页面
- “切换订单”操作：可以查看该商户不同的订单信息
- 点击“支付”按钮：
...
#### 用户登陆
- 用户登陆功能
  - 用户登陆成功：输入“用户的用户名称”，选择终端类型为用户端，点击“登陆按钮”，页面会跳转进入“扫描订单”页面
  - 用户登陆失败：“用户的用户名称”文本框不输入数据，选择终端类型为用户端，登陆页面的“登陆按钮”不可以点击。

#### 用户扫描订单
- 注：已完成的订单状态包括：订单支付成功、订单支付失败、取消订单
- “待支付”按钮：未支付完成的订单
  - 当用户没有未完成支付的订单时，“待支付”按钮不显示
  - 当用户存在至少一个未完成支付的订单时，“待支付”按钮显示，并显示待支付订单的个数
  - 点击“待支付”按钮，页面跳转进入“订单页面”
- “加入”按钮：匹配商户端创建的订单操作
  - 在文本框内录入订单号操作
  - 点击“加入”按钮
