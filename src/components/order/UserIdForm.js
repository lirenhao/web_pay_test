 ...
 */
const UserIdField = ({input, meta:{touched, error}}) => (
	<FormGroup controlId="userId">
		<InputGroup>
			<InputGroup.Addon>
				<Glyphicon glyph="qrcode"/>
			</InputGroup.Addon>
			<FormControl {...input} type="text" placeholder="用户ID"/>
			<InputGroup.Button>
				<Button type="submit" style={{"height": "34px"}} disabled={error ? true : false}>扫描用户</Button>
			</InputGroup.Button>
		</InputGroup>
	</FormGroup>
)
 ...
 */
const UserIdForm = (props) => {
	const {handleSubmit, onSubmit, orderId} = props
	return (
		<Form inline onSubmit={handleSubmit(onSubmit(orderId))}>
